...
int main() {
	...
	list = (char *)malloc(i + 1);		//выделение нужного объёма памяти		
...
void WriteList(PNode P, char *f, int i) { //Посимвольное шифрование, запись в статический массив (привычнее работать)
	while (P != NULL) {
		i--;
		f[i] = P->Sym;
